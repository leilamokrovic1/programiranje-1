...
let rec repeat elt k = if k <= 0 then [] else elt :: repeat elt (k-1)
...
let rec unzip list = 
  match list with
  | [] -> ([], [])
  | (a, b) :: xs -> let (list1, list2) = unzip xs in (a :: list1, b :: list2)
(*----------------------------------------------------------------------------*]
...
let unzip_tlrec list =
  let rec unzip_tlrec_aux list acc1 acc2 = 
  match list with
  | [] -> (reverse acc1, reverse acc2)
  | (a, b) :: xs -> unzip_tlrec_aux xs ( a :: acc1) (b :: acc2) in 
...
let apply_sequence f x n =
  ...
  else  apply_sequence_aux f (f x) (n-1) (x ::acc) in
